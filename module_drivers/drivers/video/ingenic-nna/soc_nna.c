...
static int soc_nna_mmap(struct file *file, struct vm_area_struct *vma)
{
	...
	// O_SYNC or iomem, should mapped to uncached (accelerated)
	dev_info(mdev->this_device, "(file->f_flags & O_SYNC)=0x%x, __pa(high_memory)=0x%x\n",  (file->f_flags & O_SYNC), (unsigned int)__pa(high_memory));
    if ((file->f_flags & O_SYNC) || ((paddr_start >= 0x10000000) && (paddr_end <= 0x20000000))) {
        pgprot_val(vma->vm_page_prot) &= ~_CACHE_MASK;
		// nna dma descript memory or oram memory mmaped to be uncached accelerated
		if (((paddr_start >= 0x12500000) && (paddr_end <= 0x12504000)) || ((paddr_start >= 0x12600000) && (paddr_end <= 0x12700000))) {
			pgprot_val(vma->vm_page_prot) |= _CACHE_UNCACHED_ACCELERATED;
			dev_info(mdev->this_device, "%s(%d) [%d:%d]: uncached accelerated, file->f_flags=0x%x, vma->vm_page_prot=0x%x, paddr=0x%x\n",
					__func__, __LINE__, current->tgid, current->pid, file->f_flags, (unsigned int)pgprot_val(vma->vm_page_prot), (unsigned int)paddr_start);
		} else {
			pgprot_val(vma->vm_page_prot) |= _CACHE_UNCACHED;
			dev_info(mdev->this_device, "%s(%d) [%d:%d]: uncached, file->f_flags=0x%x, vma->vm_page_prot=0x%x, paddr=0x%x\n",
					__func__, __LINE__, current->tgid, current->pid, file->f_flags, (unsigned int)pgprot_val(vma->vm_page_prot), (unsigned int)paddr_start);
		}
    } else {
        pgprot_val(vma->vm_page_prot) &= ~_CACHE_MASK;
        pgprot_val(vma->vm_page_prot) |= _CACHE_CACHABLE_WB_WA;
			dev_info(mdev->this_device, "%s(%d) [%d:%d]: cached wbwa, file->f_flags=0x%x, vma->vm_page_prot=0x%x, paddr=0x%x\n",
					__func__, __LINE__, current->tgid, current->pid, file->f_flags, (unsigned int)pgprot_val(vma->vm_page_prot), (unsigned int)paddr_start);
    }
